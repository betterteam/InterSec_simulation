...
class IM():
    def __init__(self):
        ...
        self.server_address = ('localhost', 6791)
        self.max_size = 4096
    ...
    def sendResult(self):
        while self.STOP_CHAT:
            ...
            origin = tuple(recData["origin_3_1"])
            destination = tuple(recData["destination_3_1"])
            speed = recData["speed"]
    ...
    # vehicles travel from 3W2 to 3N2
    # origin and destination is a pattern of (x,y)
    def light_veh_pattern5(self, veh_num, current, origin, destination, speed, current_time):
        ...
        while new_position[0] < destination[0]:
            ...
            # Check if all parts of veh have been in intersection
            if new_position[0] + speed <= origin[0] + 2:
                if not self.intersec_grid[time][(280, 610)]:
                    print('firstgrid')
                    return False
                else:
                    new_position = (new_position[0] + speed, new_position[1])
                ...
                # All parts of veh have been in intersection
                # Calculate rotation angle
                if 90.0 > ((new_position[0] - 270 + speed) / 14) * 90 > 15.0:
                    self.r[veh_num] = -((new_position[0] - 270 + 2) / 14) * 90
                else:
                    self.r[veh_num] = 0

                if ((new_position[0] - 270 + speed) / 14) * 90 > 90:
                    self.r[veh_num] = -90

                # Calculate trajectory by using Bezier Curve
                x = pow(1 - (self.beze_t[veh_num] / 14), 2) * 270 + 2 * (self.beze_t[veh_num] / 14) * (
                    1 - self.beze_t[veh_num] / 14) * 284 + pow(
                    self.beze_t[veh_num] / 14, 2) * 284
                y = pow(1 - (self.beze_t[veh_num] / 14), 2) * 613 + 2 * (self.beze_t[veh_num] / 14) * (
                    1 - self.beze_t[veh_num] / 14) * 613 + pow(
                    self.beze_t[veh_num] / 14, 2) * 600
                ...
                # Calculate the big Square's coordinate
                self.up_left_x[veh_num] = rec_funcs.W2N_up_left_x(new_position[0])
                self.up_left_y[veh_num] = rec_funcs.W2N_up_left_y(new_position[1], self.r[veh_num])
                self.down_left_x[veh_num] = rec_funcs.W2N_down_left_x(new_position[0])
                self.down_left_y[veh_num] = rec_funcs.W2N_down_left_y(new_position[1], self.r[veh_num])
                self.up_right_x[veh_num] = rec_funcs.W2N_up_right_x(new_position[0], self.r[veh_num])
                self.up_right_y[veh_num] = rec_funcs.W2N_up_right_y(new_position[1], self.r[veh_num])
                self.down_right_x[veh_num] = rec_funcs.W2N_down_right_x(new_position[0], self.r[veh_num])
                self.down_right_y[veh_num] = rec_funcs.W2N_down_right_y(new_position[1], self.r[veh_num])
